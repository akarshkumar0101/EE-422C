...
perform_test_on_this_submission()
{
  ...
  do
    ...
    p_dir=$(echo $t_dir | sed -e s/_/\ /g -e s/\\/$//g -e s/.\*\\///g -e s/\\b\\\(.\\\)/\\u\\1/g)
    ...
    echo "Test $p_dir" >> output_diff.txt
    echo "--------------------------------------------------------------------------------" >> output_diff.txt

    diff_result=false
    if [[ $TA_MODE == false ]]; then
      echo "Expected Output                                                    Actual Output" >> output_diff.txt
      echo "--------------------------------------------------------------------------------" >> output_diff.txt
      diff -wBytN temp_output.txt --label "Expected Output" actual_output.txt --label "Actual Output" >> output_diff.txt && diff_result=true
    else
      diff -wBuN temp_output.txt --label "Expected Output" actual_output.txt --label "Actual Output" >> output_diff.txt && diff_result=true
    fi

    if [[ $diff_result == true ]]; then
      echo "--------------------------------------------------------------------------------" >> output_diff.txt
      echo "Test Succeeded" >> output_diff.txt
      echo "--------------------------------------------------------------------------------" >> output_diff.txt
      correct_num=$(($correct_num+1))
    else
      echo "--------------------------------------------------------------------------------" >> output_diff.txt
      echo "Test Failed" >> output_diff.txt
      echo "--------------------------------------------------------------------------------" >> output_diff.txt
      if [[ $TA_MODE == false ]]; then
        echo "Error on $p_dir"
      fi
